...
using static PUBGVisionTest.Core.Helpers.NativeMethods;

namespace PUBGVisionTest.Core.Capture;

public class ScreenshotHelper
{
    // ===== Мінімальний DX11-стан (одна зона на primary screen) =====
    private static readonly Lock _dxLock = new();
    ...
    private static void EnsureDxForPrimaryScreen()
    {
        lock (_dxLock)
        {
            ...
            bool needInit =
                _dxService == null ||
                _dxZone == null ||
                _dxWidth != screenWidth ||
                _dxHeight != screenHeight;
            ...
            var display = displays
                .Where(d => d.Width == screenWidth && d.Height == screenHeight)
